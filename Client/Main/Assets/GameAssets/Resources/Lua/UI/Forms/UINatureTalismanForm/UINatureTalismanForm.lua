...
--作者： xc
--日期： 2019-04-18
--文件： UINatureTalismanForm.lua
--模块： UINatureTalismanForm
--描述： 造化翅膀面板
------------------------------------------------
--引用
...
function UINatureTalismanForm:OnRegisterEvents()
	self:RegisterEvent(UIEventDefine.UINatureTalismanForm_OPEN, self.OnOpen)
    self:RegisterEvent(UIEventDefine.UINatureTalismanForm_CLOSE,self.OnClose)
    self:RegisterEvent(LogicEventDefine.NATURE_EVENT_TALISMAN_INIT, self.UpDateTalismanInfo)
    self:RegisterEvent(LogicEventDefine.NATURE_EVENT_TALISMAN_UPLEVEL, self.UpDateTalismanUpLevel)
    self:RegisterEvent(LogicEventDefine.NATURE_EVENT_FASHION_CHANGEMODEL,self.UpDateChangeModel)
    self:RegisterEvent(LogicEventDefine.NATURE_EVENT_TALISMAN_UPDATEDRUG, self.UpDateChangeDrug)
end
--打开
function UINatureTalismanForm:OnOpen(obj, sender)
...
--关闭
function UINatureTalismanForm:OnClose(obj,sender)
    self.CSForm:Hide()
end

function UINatureTalismanForm:OnFirstShow()
    self:FindAllComponents()
    self:RegUICallback()
    self:InitRedPoint()
end

function UINatureTalismanForm:OnShowBefore()
    if  GameCenter.NatureSystem.NatureTalismanData.Cfg == nil then 
        return
    end
    self.RedPoint:Initialize()
    self.CurShowModel = GameCenter.NatureSystem.NatureTalismanData.super:GetCurShowModel()
    self:SetModel() --刷新模型
    self:SetItem()
...
--刷新界面数据
function UINatureTalismanForm:RefreshLevelInfo()
    self.SkillsInfo:RefreshSkill(GameCenter.NatureSystem.NatureTalismanData.super.SkillList) --刷新技能显示
    self:SetAttr() --刷新属性显示
    self:SetModelButton()
    self:SetMaxLevel()
    self:SetSlider()
...
--查找组件
function UINatureTalismanForm:FindAllComponents()
    self.UIListMenu = UIListMenu:OnFirstShow(self.CSForm, UIUtils.FindTrans(self.Trans, "Right/UIListMenu"))
    self.UIListMenu:AddIcon(NatureSubEnum.Drug,DataConfig.DataMessageString.Get("NATURETALISMANTYPETWO"),FunctionStartIdCode.NatureTalismanDrug)
    self.UIListMenu:AddIcon(NatureSubEnum.BaseUpLevel,DataConfig.DataMessageString.Get("NATURECULTIVATE"),FunctionStartIdCode.NatureTalismanLevel)
...
--注册UI上面的事件，比如点击事件等
function UINatureTalismanForm:RegUICallback()
...
function UINatureTalismanForm:OnHideBefore()
    if self.PlayerSkin then
...
--分页选择
function UINatureTalismanForm:OnMenuSelect(id, sender)
    self.Form = id
    if sender then
...
--点击左边
function UINatureTalismanForm:OnClickLeft()
    local _lastmodel = GameCenter.NatureSystem.NatureTalismanData.super:GetLastModel(self.CurShowModel)
...
--点击右边
function UINatureTalismanForm:OnClickRight()
    local _lastmodel = GameCenter.NatureSystem.NatureTalismanData.super:GetNextModel(self.CurShowModel)
    if _lastmodel ~= 0 then
        self.CurShowModel = _lastmodel
        self:SetModel()
        self:SetModelButton()
...
--点击切换模型
function UINatureTalismanForm:OnClickShowModel()
    GameCenter.NatureSystem:ReqNatureModelSet(NatureEnum.Talisman,self.CurShowModel)
end

--按钮点击事件一键升级
function UINatureTalismanForm:OnClickOneKey()
    self:SendUpLevel(true)
end

--按钮点击事件升级
function UINatureTalismanForm:OnClickUpLevel()
    self:SendUpLevel(false)
end

function UINatureTalismanForm:SendUpLevel(isonekey)
    if self.CurSelectItemId ~= 0 then
        local _haveNum = GameCenter.ItemContianerSystem:GetItemCountFromCfgId(self.CurSelectItemId)
        if _haveNum > 0 then
            --发送消息
            GameCenter.NatureSystem:ReqNatureUpLevel(NatureEnum.Talisman,self.CurSelectItemId,isonekey)
        else
...
--时装按钮
function UINatureTalismanForm:OnClickFishion()
    GameCenter.PushFixEvent(UIEventDefine.UINatureFashionForm_OPEN,NatureEnum.Talisman)
end

--模型外显按钮
function UINatureTalismanForm:OnClickModelShow()
    GameCenter.PushFixEvent(UIEventDefine.UINatureModelShowForm_OPEN,NatureEnum.Talisman)
end

--添加红点进入RedPoint组件
function UINatureTalismanForm:InitRedPoint()
    self.RedPoint = CommonPanelRedPoint:New()
    self.RedPoint:Add(FunctionStartIdCode.NatureTalismanLevel,self.OneKeyButton.transform,NatureSubEnum.Begin,true)
    self.RedPoint:Add(FunctionStartIdCode.NatureTalismanLevel,self.UpLevelButton.transform,NatureSubEnum.Begin,true)
    self.RedPoint:Add(FunctionStartIdCode.NatureTalismanFashion,self.FishionButton.transform,NatureSubEnum.Begin,false)
end

--设置进度条
function UINatureTalismanForm:SetSlider()
    self.ProssSlider.value = GameCenter.NatureSystem.NatureTalismanData.super.CurExp / GameCenter.NatureSystem.NatureTalismanData.Cfg.Progress
    self.ProssLab.text = string.format("%d/%d",GameCenter.NatureSystem.NatureTalismanData.super.CurExp,GameCenter.NatureSystem.NatureTalismanData.Cfg.Progress)
    self.ProssLevelLab.text = tostring(GameCenter.NatureSystem.NatureTalismanData.super.Level)
end

--设置是否满级
function UINatureTalismanForm:SetMaxLevel()
    self.IsMaxGo.gameObject:SetActive(GameCenter.NatureSystem.NatureTalismanData:IsMaxLevel())
end

--设置模型翅膀
function UINatureTalismanForm:SetModel()
    if self.CurShowModel ~= 0 then
        ...
        self.PlayerSkin:SetCameraSize(GameCenter.NatureSystem.NatureTalismanData:Get3DUICamerSize(self.CurShowModel))
        self.PlayerSkin.EnableDrag = true
        self.ModelName.text = GameCenter.NatureSystem.NatureTalismanData.super:GetModelsName(self.CurShowModel)
    else
...
--设置模型切换按钮状态
function UINatureTalismanForm:SetModelButton()
    local _isleft = GameCenter.NatureSystem.NatureTalismanData.super:GetNotLeftButton(self.CurShowModel)
    local _isright = GameCenter.NatureSystem.NatureTalismanData.super:GetNotRightButton(self.CurShowModel)
    self.LeftModelButton.gameObject:SetActive(_isleft)
    self.RightModelButton.gameObject:SetActive(_isright)
    self.ShowModelButton.gameObject:SetActive(self.CurShowModel ~= GameCenter.NatureSystem.NatureTalismanData.super.CurModel)
end

--设置属性
function UINatureTalismanForm:SetAttr()
    local _attrlist = GameCenter.NatureSystem.NatureTalismanData.super.AttrList
...
--设置吃果子信息
function UINatureTalismanForm:SetDrugInfo()
    local _druglist = GameCenter.NatureSystem.NatureTalismanData.super.DrugList
...
--点击吃果子
function UINatureTalismanForm:OnClickDrugBtn(go)
    ...
    if _haveNum > 0 then
       GameCenter.NatureSystem:ReqNatureDrug(NatureEnum.Talisman,_itemId)
    else
...
--设置道具显示
function UINatureTalismanForm:SetItem()
    local _itemlist = GameCenter.NatureSystem.NatureTalismanData.super.ItemList
...
--选择道具
function UINatureTalismanForm:OnClickItemBtn(go)
...
--播放特效
function UINatureTalismanForm:PlayerVfx()
    if self.Effect then
        self.Effect:Play()
    end
end
...
--设置战斗力
function UINatureTalismanForm:SetFight()
    self.FightLab.text = tostring(GameCenter.NatureSystem.NatureTalismanData.super.Fight)
end

--网络消息过来刷新面板
function UINatureTalismanForm:UpDateTalismanInfo()
    self.CurShowModel = GameCenter.NatureSystem.NatureTalismanData.super.GetCurShowModel()
    self:SetModel() --刷新模型
    self:SetItem()
    self:RefreshLevelInfo()
...
--网络消息升级
function UINatureTalismanForm:UpDateTalismanUpLevel(oldlevel,sender)
    self:RefreshLevelInfo()
    if oldlevel < GameCenter.NatureSystem.NatureTalismanData.super.Level then
        self:PlayerVfx()
    end
end

--网络消息切换模型
function UINatureTalismanForm:UpDateChangeModel(type)
    if type == NatureEnum.Talisman then
        self:SetModelButton()
    end
end

--网络消息更新果子信息
function UINatureTalismanForm:UpDateChangeDrug()
    self:SetFight()
    self:SetDrugInfo()
end

--更新特效
function UINatureTalismanForm:Update(dt)
    if self.Effect then
        self.Effect:Tick(dt)
    end
end

return UINatureTalismanForm