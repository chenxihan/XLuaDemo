...
public class UIWidgetInspector : UIRectEditor
{
	...
	static public void DrawInspectorProperties (SerializedObject so, UIWidget w, bool drawColor)
	{
		...
		if (NGUIEditorTools.DrawHeader("Widget"))
		{
            ...
            GUILayout.BeginHorizontal();
            {
                w.IsGray = EditorGUILayout.Toggle("IsGray",w.IsGray, GUILayout.Width(100f));
                GUILayout.Label("use gray effect");                                
            }
            GUILayout.EndHorizontal();

            GUILayout.BeginHorizontal();
            {
                w.IsFlowLight = EditorGUILayout.Toggle("IsFlowLight", w.IsFlowLight, GUILayout.Width(100f));                
                GUILayout.Label("use flowLight effect");
                w.IsOverall = EditorGUILayout.Toggle("IsOverall", w.IsOverall, GUILayout.Width(100f));
            }
            GUILayout.EndHorizontal();
#endif
