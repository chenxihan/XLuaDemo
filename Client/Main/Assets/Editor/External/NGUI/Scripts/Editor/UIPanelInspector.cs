	...
	protected override bool ShouldDrawProperties ()
	{
		...
		if (mPanel.clipping != UIDrawCall.Clipping.None)
		{
			...
			if (mPanel.clipping == UIDrawCall.Clipping.SoftClip)
			{
                ...
                GUILayout.BeginHorizontal();
                GUILayout.Space(80f);
                Vector4 softHide = EditorGUILayout.Vector4Field("SoftnessHide(Left,Right,Top,Bottom)", mPanel.ClipSoftnessHide, GUILayout.MinWidth(20f));
                GUILayout.EndHorizontal();               

                if (mPanel.ClipSoftnessHide != softHide)
                {
                    NGUIEditorTools.RegisterUndo("Clipping Change", mPanel);
                    mPanel.ClipSoftnessHide = softHide;
                    EditorUtility.SetDirty(mPanel);
                }
#endif
			}
