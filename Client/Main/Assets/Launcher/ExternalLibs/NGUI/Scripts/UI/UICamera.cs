...
public class UICamera : MonoBehaviour
{
    ...
    public string _horizontalAxisName = "Horizontal";
    ...
    private string _verAxis = "Vertical";
    ...
    //add by gzg,2016-01-13        
	/// <summary>
	/// If set, this game object will receive all events regardless of whether they were handled or not.
	/// </summary>
    static List<GameObject> mGenericHandlerList = new List<GameObject>();
    static public void AddGenericEventHandler(GameObject genericEventHandler)
    {
        if (genericEventHandler!=null && mGenericHandlerList.IndexOf(genericEventHandler) < 0)
        {
            mGenericHandlerList.Add(genericEventHandler); 
        }
    }
    static public void RemoveGenericEventHandler(GameObject genericEventHandler)
    {
        if (genericEventHandler != null)
        {
            mGenericHandlerList.Remove(genericEventHandler);
        }
    }
	...
	static public void Raycast (MouseOrTouch touch)
	{
		...
		if (mRayHitObject == null) mRayHitObject = null;
	...
	static public void Notify (GameObject go, string funcName, object obj)
	{
        ...
        //add by gzg,2016-01-13        
        if (mGenericHandlerList.Count > 0)
        {
            for (int i = 0; i < mGenericHandlerList.Count; i++)
            {
                if (mGenericHandlerList[i] != null && mGenericHandlerList[i] != go)
                {
                    mGenericHandlerList[i].SendMessage(funcName, obj, SendMessageOptions.DontRequireReceiver);
                }
            }
        }
	}
		...
		if (currentTouch.pressed != null)
		{
				...
				currentTouch.totalDelta.sqrMagnitude < drag))
			{
				...
				if (currentTouch.clickNotification != ClickNotification.None && currentTouch.pressed == currentTouch.current)
				{
					...
					if (currentTouch.clickTime + 0.35f > time && currentTouch.clickObj == currentTouch.pressed)
					{
                    ...
                    currentTouch.clickObj = currentTouch.pressed;
                    if (isDouble)
                        currentTouch.clickObj = null;
