...
public class UIDrawCall : MonoBehaviour
{
    ...
    [HideInInspector][System.NonSerialized] public BetterList<Vector2> uv2s = new BetterList<Vector2>();
	...
	void CreateMaterial ()
	{
        ...
        string shaderName = (mShader != null) ? mShader.name :
            ((mMaterial != null) ? mMaterial.shader.name : "Unlit/Transparent Colored_RGB");
#else
        string shaderName = (mShader != null) ? mShader.name :
		...
		if (count > 0 && (count == uvs.size && count == cols.size) && (count % 4) == 0)
		{
			...
			if (verts.size < 65000)
			{
				...
				bool trim = (uvs.buffer.Length != verts.buffer.Length) ||
                    (uv2s.buffer.Length != verts.buffer.Length) ||
	...
	const int maxIndexBufferCache = 300;
    ...
    public static void PrintCache()
    {
        ...
        {
            sb.AppendLine("i="+i+";;len:"+mCache[i].Length);
            sum += mCache[i].Length;
        }
...
#if FUNCELL_MODIFIED
                    SetClipping(i++, cr, currentPanel.clipSoftness,currentPanel.ClipSoftnessHide,angle);
#else
...
#endif
	{
		...
		if (index < ClipRange.Length)
		{
            ...
            {
                //Debug.Log(";;;" + softHide);
                mDynamicMat.SetVector("_ClipSoftnessHide", softHide);
            }
#endif
		}
	}
