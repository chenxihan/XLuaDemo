...
public class UIGrid : UIWidgetContainer
{
...
#if FUNCELL_MODIFIED
    /// <summary>
    /// 针对第二层排列时,使用的是从左到右,还是从右到左
    /// </summary>
    public bool LeftToRightLine = true;
#endif
    ...
    //调整位置需要等待的帧数,默认不等待当前帧进行处理
    public int repositionWaitFrameCount = 0;
	...
	{
        if (repositionWaitFrameCount > 0)
        {
            repositionWaitFrameCount--;
            return;
        }
        Reposition();
		enabled = false;
	...
	protected virtual void ResetPosition (List<Transform> list)
	{
		...
		for (int i = 0, imax = list.Count; i < imax; ++i)
		{
            ...
            {
#if FUNCELL_MODIFIED
                //判断是否为第一行,是否是从左到右的排列方式
                if (isFirstRow || LeftToRightLine)
                {
#endif
                ...
                {
                    pos = (arrangement == Arrangement.Horizontal) ?
                       new Vector3(cellWidth * (maxX - x), -cellHeight * y, depth) :
                       new Vector3(cellWidth * y, -cellHeight * (maxX - x), depth);
                }
#endif
...
#if FUNCELL_MODIFIED
            //注释掉后面的判断位置的检测,因为transform的list中对象被修改名字后,重新排序,出现原来的位于(0,0,0)点的对象,现在排在索引第一个后,不会被重新排序的问题.
